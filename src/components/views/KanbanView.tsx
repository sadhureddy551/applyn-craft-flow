...
export function KanbanView({ records, fields, moduleId, onView, onDelete, onUpdateRecord }: KanbanViewProps) {
  const { toast } = useToast();
  const [draggedId, setDraggedId] = useState<string | null>(null);

  // Get stages from pipeline or from select field options
            ...
            <div className="space-y-2 min-h-[100px] rounded-lg bg-muted/30 border border-border/50 p-2">
              {stageRecords.map((rec) => (
                <motion.div
                  ...
                  animate={{ opacity: draggedId === rec.id ? 0.5 : 1, scale: 1 }}
                  draggable
                  onDragStart={(e: any) => handleDragStart(e, rec.id)}
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="h-5 w-5 flex items-center justify-center rounded-sm opacity-0 group-hover:opacity-100 hover:bg-muted transition-all">
                          <MoreHorizontal className="h-3 w-3 text-muted-foreground" />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => onView(rec.id)}>
                          <Eye className="h-3.5 w-3.5 mr-2" /> View
                        </DropdownMenuItem>
                        <DropdownMenuItem className="text-destructive" onClick={() => onDelete(rec.id, rec.values?.[nameField?.fieldKey] || 'Untitled')}>
                  ...
                  <div className="mt-2 space-y-1.5">
                    {companyField && rec.values?.[companyField.fieldKey] && (
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="truncate">{rec.values[companyField.fieldKey]}</span>
                      </div>
                    )}
                    {amountField && rec.values?.[amountField.fieldKey] != null && (
                      <div className="flex items-center gap-1.5 text-xs font-medium text-foreground">
                        <DollarSign className="h-3 w-3 text-muted-foreground" />
                        ${Number(rec.values[amountField.fieldKey]).toLocaleString()}
                      </div>
                    )}
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <User className="h-3 w-3" />
                      <span>{rec.createdBy}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
