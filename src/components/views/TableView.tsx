...
export function TableView({
  ...
  onView, onDelete, visibleColumns, onVisibleColumnsChange,
}: TableViewProps) {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="h-7 text-xs">
                <Columns className="h-3 w-3 mr-1.5" /> Columns
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              ...
              {fields.map((f) => (
                <DropdownMenuCheckboxItem
                  key={f.id}
                  checked={visible.includes(f.fieldKey)}
                  onCheckedChange={(checked) => {
                    const next = checked
                      ? [...visible, f.fieldKey]
                      : visible.filter((k) => k !== f.fieldKey);
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem className="text-destructive" onClick={() => {
                        ...
                        onDelete(rec.id, rec.values?.[nameField?.fieldKey] || 'Untitled');
                      }}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-muted/20">
          ...
          <div className="flex gap-1">
            ...
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => (
              <Button key={i} variant={page === i + 1 ? 'default' : 'ghost'} size="icon" className="h-7 w-7 text-xs" onClick={() => onPageChange(i + 1)}>
                {i + 1}
              </Button>
            ))}
